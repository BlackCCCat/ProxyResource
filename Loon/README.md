...
```
定时任务插件（按节点名检测，可靠） → 写入缓存 → 解析器读缓存改名
```

解析器设成 `auto` 时会先尝试实测，路由自检不通过就自动退回缓存，不会白白丢结果；
但既然知道结果，直接用 `cache` 省掉那一轮无谓的等待。

> **顺序很重要**：必须先让定时任务插件成功跑过一次（打开插件手动触发即可），
> 缓存里有东西了，解析器才有的可改。缓存是空的时候解析器会在日志里明确告诉你。
...
| `本次没有任何可用的检测结果` + `缓存是空的` | 定时任务插件没成功跑过 | 打开该插件手动触发一次，再更新订阅 |
| `缓存里有 N 条记录，但和本订阅的节点名一条都对不上` | 定时任务的 `group` 不是这个订阅所在的组 | 按日志打印的两组样本名对照修正 `group` |
| `展开策略组「X」：…得到 0 个真实节点` | `group` 填错了 | 日志会列出配置里所有策略组名，照着填 |

**"所有节点都不支持"** 和 **"所有节点都可用"** 是同一个病的两面：
前者是拿不到结果（缓存空 / 名字对不上），后者是所有请求走了同一个出口。
先看日志第一行的 `展开策略组` 和 `路由自检`，基本就定位了。
