...
| `Plugin/AI-Unlock-Check.plugin` | **定时任务插件**。自动汇总解析器登记的多份节点订阅并复检，写入缓存；可自动切换 AI 策略组；也可手动点单个节点看检测面板 |
...
不需要填写策略组。把解析器挂到每份需要检测的节点订阅上，并至少更新一次订阅；定时任务会自动读取这些订阅登记的真实节点。
...
**推荐组合（也是默认配置）**：两个插件都装，解析器用 `source=cache`。

实测下来，解析阶段拿「节点描述」当出口这条路在不少环境里不生效——请求会悄悄回落到当前默认策略，
于是每个节点测出来的结果一模一样（全可用或全不可用）。而**按节点名**检测是可靠的，
「定时任务」插件走的就是这条路。解析器在每次订阅更新时还会登记其真实节点名，所以：

```
更新已启用解析器的订阅（登记节点） → 定时任务按节点名检测 → 写入缓存 → 再次更新订阅并打标
```

解析器设成 `auto` 时会先尝试实测，路由自检不通过就自动退回缓存，不会白白丢结果；
但既然知道结果，直接用 `cache` 省掉那一轮无谓的等待。

> **首次使用顺序很重要**：先更新所有启用了本解析器的节点订阅，让脚本登记节点；然后手动触发一次定时任务；最后再更新一次订阅完成打标。以后由 cron 定期刷新检测缓存。
...
| `没有已登记的订阅节点` | 安装新版后还没运行解析器 | 给待检测订阅启用本解析器，并逐个更新一次 |
| `本次没有任何可用的检测结果` + `缓存是空的` | 已登记节点，但定时任务还没成功跑过 | 打开定时任务插件手动触发一次，再更新订阅 |
| `缓存里有 N 条记录，但和本订阅的节点名一条都对不上` | 订阅节点名称已经变化或缓存过期 | 先更新订阅登记新名称，再运行定时任务，最后再次更新订阅 |

**"所有节点都不支持"** 和 **"所有节点都可用"** 是同一个病的两面：
前者是拿不到结果（缓存空 / 名字对不上），后者是所有请求走了同一个出口。
先看日志里的 `读取 N 份订阅，共 M 个节点` 和 `路由自检`，基本就定位了。每份订阅还会单独打印脱敏后的域名、短标识和节点数，不会输出订阅 URL、路径或 token。

路由自检未通过时的三条处理办法：

1. 给每份待检测订阅启用解析器并更新一次，再装「AI 解锁检测 · 定时任务」插件；它会按登记的**节点名**检测；
